...
void Preprocess::process_cut_frame_livox(
  ...
  for (uint i = 1; i < plsize; i++) {
    if ((msg->points[i].line < N_SCANS) &&
      ...
      if (valid_point_num % point_filter_num == 0) {
        ...
        double dist = pl_full[i].x * pl_full[i].x +
                      pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z;
        if (dist < blind)
          continue;
  ...
  for (uint i = 1; i < valid_pcl_size; i++) {
    ...
    pl_surf[i].curvature +=
        msg->header.stamp.toSec() * 1000 - last_frame_end_time;
...
void Preprocess::velodyne_handler_nclt(
    const sensor_msgs::PointCloud2::ConstPtr &msg) {
