...
std::vector<int> ground_classes = {
    ROAD, PARKING, SIDEWALKR, OTHER_GROUND, LANE_MARKING, VEGETATION, TERRAIN};
std::vector<int> ground_classes_except_terrain = {ROAD, PARKING, SIDEWALKR,
                                                  OTHER_GROUND, LANE_MARKING};
std::vector<int> traversable_ground_classes = {ROAD, PARKING, LANE_MARKING,
                                               OTHER_GROUND};
...
int count_num_ground(const pcl::PointCloud<PointXYZILID> &pc) {
  ...
  for (auto const &pt : pc.points) {
    ...
    if (iter !=
        ground_classes.end()) { // corresponding class is in ground classes
...
int count_num_ground_without_vegetation(
    const pcl::PointCloud<PointXYZILID> &pc) {
  int num_ground = 0;

  std::vector<int>::iterator iter;

  std::vector<int> classes = {ROAD,         PARKING,      SIDEWALKR,
...
std::map<int, int>
count_num_each_class(const pcl::PointCloud<PointXYZILID> &pc) {
  int num_ground = 0;
...
int count_num_outliers(const pcl::PointCloud<PointXYZILID> &pc) {
  ...
  for (auto const &pt : pc.points) {
    ...
    if (iter !=
        outlier_classes.end()) { // corresponding class is in ground classes
      num_outliers++;
    }
  }
  return num_outliers;
...
void discern_ground(const pcl::PointCloud<PointXYZILID> &src,
...
void calculate_precision_recall_without_vegetation(
    const pcl::PointCloud<PointXYZILID> &pc_curr,
    pcl::PointCloud<PointXYZILID> &ground_estimated, double &precision,
    double &recall, bool consider_outliers = true) {
  int num_veg = 0;
...
int save_all_labels(const pcl::PointCloud<PointXYZILID> &pc, string ABS_DIR,
                    string seq, int count) {

  std::string count_str = std::to_string(count);
  std::string count_str_padded =
      std::string(NUM_ZEROS - count_str.length(), '0') + count_str;
  std::string output_filename =
      ABS_DIR + "/" + seq + "/" + count_str_padded + ".csv";
...
void save_all_accuracy(const pcl::PointCloud<PointXYZILID> &pc_curr,
                       pcl::PointCloud<PointXYZILID> &ground_estimated,
                       string acc_filename, double &accuracy,
                       map<int, int> &pc_curr_gt_counts,
  ...
  accuracy =
      static_cast<double>(num_TP + (num_False - num_FP)) / num_total_gt * 100.0;
...
void pc2pcdfile(const pcl::PointCloud<PointXYZILID> &TP,
                const pcl::PointCloud<PointXYZILID> &FP,
                const pcl::PointCloud<PointXYZILID> &FN,
                const pcl::PointCloud<PointXYZILID> &TN,
