...
void KD_TREE::InitializeKDTree(float delete_param, float balance_param,
                               float box_length) {
...
void KD_TREE::Nearest_Search(PointType point, int k_nearest,
...
int KD_TREE::Delete_by_range(KD_TREE_NODE **root, BoxPointType boxpoint,
...
void KD_TREE::Add_by_range(KD_TREE_NODE **root, BoxPointType boxpoint,
                           bool allow_rebuild) {
  if ((*root) == nullptr)
    return;
  (*root)->working_flag = true;
...
void KD_TREE::Add_by_point(KD_TREE_NODE **root, PointType point,
...
void KD_TREE::Search_by_range(KD_TREE_NODE *root, BoxPointType boxpoint,
                              PointVector &Storage) {
  if (root == nullptr)
    return;
  Push_Down(root);
...
MANUAL_HEAP::~MANUAL_HEAP() { delete[] heap; }
...
PointType_CMP MANUAL_HEAP::top() { return heap[0]; }
...
int MANUAL_HEAP::size() { return heap_size; }
...
Operation_Logger_Type MANUAL_Q::front() { return q[head]; }

Operation_Logger_Type MANUAL_Q::back() { return q[tail]; }
...
bool MANUAL_Q::empty() { return is_empty; }

int MANUAL_Q::size() { return counter; }
