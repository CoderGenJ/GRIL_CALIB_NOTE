...
struct StatesGroup {
  ...
  StatesGroup operator+(const Matrix<double, DIM_STATE, 1> &state_add) {
    ...
    a.offset_R_L_I = this->offset_R_L_I *
                     Exp(state_add(6, 0), state_add(7, 0), state_add(8, 0));
  ...
  StatesGroup &operator+=(const Matrix<double, DIM_STATE, 1> &state_add) {
    ...
    this->offset_R_L_I = this->offset_R_L_I *
                         Exp(state_add(6, 0), state_add(7, 0), state_add(8, 0));
...
template <typename T> T SI2g(T SI) { return SI / G_m_s2; }

// Convert g unit to SI unit
template <typename T> T g2SI(T g) { return g * G_m_s2; }

// So(2) rotation matrix
// theta - radian
template <typename T> M2D SO2(T theta) {
  M2D R;
...
// Skew matrix 2D
// theta - radian
template <typename T> M2D Skew2D(T theta) {
  M2D exp;
  exp << 0, -theta, theta, 0;
  return exp;
}
