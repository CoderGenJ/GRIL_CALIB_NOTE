...
Gril_Calib::Gril_Calib()
    : time_delay_IMU_wtr_Lidar(0.0), time_lag_1(0.0), time_lag_2(0.0),
      lag_IMU_wtr_Lidar(0) {
...
void Gril_Calib::set_states_2nd_filter(const deque<CalibState> &IMU_states,
                                       const deque<CalibState> &Lidar_states) {
...
void Gril_Calib::fout_before_filter() {
  for (auto it_IMU = IMU_state_group.begin();
       it_IMU != IMU_state_group.end() - 1; it_IMU++) {
    fout_before_filt_IMU << setprecision(15) << it_IMU->ang_vel.transpose()
                         << " " << it_IMU->ang_vel.norm() << " "
                         << it_IMU->linear_acc.transpose() << " "
                         << it_IMU->timeStamp << endl;
  }
  for (auto it = Lidar_state_group.begin(); it != Lidar_state_group.end() - 1;
       it++) {
    fout_before_filt_Lidar << setprecision(15) << it->ang_vel.transpose() << " "
                           << it->ang_vel.norm() << " " << it->timeStamp
                           << endl;
  }
}
...
void Gril_Calib::fout_check_lidar() {
  ...
  for (; it_Lidar_state != Lidar_state_group.end() - 2; it_Lidar_state++) {
    fout_LiDAR_meas_after << setprecision(12)
...
void Gril_Calib::push_ALL_IMU_CalibState(const sensor_msgs::Imu::ConstPtr &msg,
                                         const double &mean_acc_norm) {
  CalibState IMUstate;
  double invert = -1.0;
  IMUstate.ang_vel = V3D(msg->angular_velocity.x, msg->angular_velocity.y,
                         msg->angular_velocity.z);
  IMUstate.linear_acc =
      V3D(msg->linear_acceleration.x, msg->linear_acceleration.y,
          msg->linear_acceleration.z) /
      mean_acc_norm * G_m_s2;

  IMUstate.timeStamp = msg->header.stamp.toSec();
  IMU_state_group_ALL.push_back(IMUstate);
}

void Gril_Calib::push_IMU_CalibState(const V3D &omg, const V3D &acc,
...
void Gril_Calib::push_Lidar_CalibState(const M3D &rot, const V3D &pos,
                                       const V3D &omg, const V3D &linear_vel,
                                       const double &timestamp) {
...
void Gril_Calib::push_Plane_Constraint(const Eigen::Quaterniond &q_lidar,
...
void Gril_Calib::downsample_interpolate_IMU(const double &move_start_time) {
  ...
  IMU_states_all_origin.assign(IMU_state_group_ALL.begin(),
  ...
  for (int i = mean_filt_size; i < IMU_state_group_ALL.size() - mean_filt_size;
    ...
    for (int k = -mean_filt_size; k < mean_filt_size + 1; k++)
      acc_real += (IMU_states_all_origin[i + k].linear_acc - acc_real) /
                  (k + mean_filt_size + 1);
  ...
  for (int i = 0; i < Lidar_state_group.size(); i++) {
    for (int j = 1; j < IMU_state_group_ALL.size(); j++) {
      if (IMU_state_group_ALL[j - 1].timeStamp <=
        ...
        double delta_t = IMU_state_group_ALL[j].timeStamp -
        ...
        push_IMU_CalibState(IMU_state_interpolation.ang_vel,
                            IMU_state_interpolation.linear_acc,
                            Lidar_state_group[i].timeStamp);
        break;
      }
...
// Calculates the angular acceleration of the IMU, LiDAR, and linear
// acceleration.
void Gril_Calib::central_diff() {
  ...
  for (; it_IMU_state != IMU_state_group.end() - 2; it_IMU_state++) {
    ...
    fout_IMU_meas << setprecision(12) << it_IMU_state->ang_vel.transpose()
...
void Gril_Calib::xcorr_temporal_init(const double &odom_freq) {
  ...
  for (int i = 0; i < N; i++) {
    mean_IMU_ang_vel +=
        (IMU_state_group[i].ang_vel.norm() - mean_IMU_ang_vel) / (i + 1);
    mean_LiDAR_ang_vel +=
        (Lidar_state_group[i].ang_vel.norm() - mean_LiDAR_ang_vel) / (i + 1);
  }
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (j < 0 || j > N - 1)
        ...
        corr += (IMU_state_group[i].ang_vel.norm() - mean_IMU_ang_vel) *
                (Lidar_state_group[j].ang_vel.norm() -
                 mean_LiDAR_ang_vel); // Zero-centered cross correlation
      }
    }
  ...
  cout << "Max Cross-correlation: IMU lag wtr Lidar : " << -lag_IMU_wtr_Lidar
       << endl;
  cout << "Time lag 1: IMU lag wtr Lidar : " << time_lag_1 << endl;
}

void Gril_Calib::IMU_time_compensate(const double &lag_time,
...
void Gril_Calib::acc_interpolate() {
  ...
  for (int i = 1; i < Lidar_state_group.size() - 1; i++) {
    ...
    if (deltaT > 0) {
      double DeltaT =
          IMU_state_group[i + 1].timeStamp - IMU_state_group[i].timeStamp;
      ...
      double DeltaT =
          IMU_state_group[i].timeStamp - IMU_state_group[i - 1].timeStamp;
      double s = -deltaT / DeltaT;
      IMU_state_group[i].linear_acc = s * IMU_state_group[i - 1].linear_acc +
                                      (1 - s) * IMU_state_group[i].linear_acc;
      IMU_state_group[i].timeStamp += deltaT;
...
void Gril_Calib::Butter_filt(const deque<CalibState> &signal_in,
                             deque<CalibState> &signal_out) {
  ...
  for (int i = butter.Coeff_size; i < sig_extended.size() - butter.extend_num;
       i++) {
  ...
  for (auto it = sig_out.begin() + butter.extend_num;
       it != sig_out.end() - butter.extend_num; it++) {
    signal_out.push_back(*it);
  }
}

// zero phase low-pass filter //
void Gril_Calib::zero_phase_filt(const deque<CalibState> &signal_in,
                                 deque<CalibState> &signal_out) {
  deque<CalibState> sig_out1;
  Butter_filt(signal_in, sig_out1); // signal_in에 대해 Butterworth filter를
                                    // 적용한 결과를 sig_out1에 저장
...
void Gril_Calib::solve_Rotation_only() {
  ...
  for (int i = 0; i < IMU_state_group.size(); i++) {
    ...
    problem_rot.AddResidualBlock(
        Angular_Vel_Cost_only_Rot::Create(IMU_state_group[i].ang_vel,
...
void Gril_Calib::solve_Rot_Trans_calib(double &timediff_imu_wrt_lidar,
                                       const double &imu_sensor_height) {

  M3D R_IL_init =
      Rot_Lidar_wrt_IMU.transpose(); // Initial value of Rotation of IL (from
  ...
  double Trans_IL[3]; // Initial value of Translation of IL (IMU with respect to
                      // Lidar) - ceres solver input
  ...
  ceres::ScaledLoss *loss_function_angular_scaled =
      new ceres::ScaledLoss(loss_function_angular, 0.5, ceres::TAKE_OWNERSHIP);

  ceres::LossFunction *loss_function_acc = new ceres::CauchyLoss(0.5);
  ceres::ScaledLoss *scaled_loss_acc =
      new ceres::ScaledLoss(loss_function_acc, 0.2, ceres::TAKE_OWNERSHIP);

  ceres::LossFunction *loss_function_plain_motion = new ceres::HuberLoss(0.5);
  ceres::ScaledLoss *loss_function_plain_motion_scaled = new ceres::ScaledLoss(
      loss_function_plain_motion, 0.3, ceres::TAKE_OWNERSHIP);
  ...
  for (int i = 0; i < IMU_state_group.size(); i++) {
    ...
    problem_Ex_calib.AddResidualBlock(
        ...
        scaled_loss_acc, R_IL_quat, bias_aL, Trans_IL);
  ...
  if (set_boundary) {
    for (int index = 0; index < 3; ++index) {
      problem_Ex_calib.SetParameterUpperBound(Trans_IL, index,
                                              Trans_IL[index] + bound_th);
      problem_Ex_calib.SetParameterLowerBound(Trans_IL, index,
                                              Trans_IL[index] - bound_th);
    }
  }
  ...
  // Rotation matrix
  Eigen::Quaterniond q_IL_final(
      R_IL_quat[0], R_IL_quat[1], R_IL_quat[2],
      R_IL_quat[3]); // quaternion from IMU frame to Lidar frame
  ...
  for (int i = 0; i < IMU_state_group.size(); i++) {
    ...
    V3D acc_I = Lidar_state_group[i].rot_end * Rot_Lidar_wrt_IMU.transpose() *
    ...
    V3D acc_L = Lidar_state_group[i].linear_acc +
                ...
                Grav_L;
    fout_acc_cost << setprecision(10) << acc_I.transpose() << " "
                  << acc_L.transpose() << " " << IMU_state_group[i].timeStamp
...
void Gril_Calib::normalize_acc(deque<CalibState> &signal_in) {
  ...
  for (int i = 0; i < signal_in.size(); i++) {
    signal_in[i].linear_acc =
        signal_in[i].linear_acc / mean_acc.norm() * G_m_s2;
  }
}

// Align the size of various states
void Gril_Calib::align_Group(const deque<CalibState> &IMU_states,
                             deque<Eigen::Quaterniond> &Lidar_wrt_ground_states,
                             deque<Eigen::Quaterniond> &IMU_wrt_ground_states,
                             deque<V3D> &normal_vector_wrt_lidar_group,
...
bool Gril_Calib::data_sufficiency_assess(MatrixXd &Jacobian_rot, int &frame_num,
                                         V3D &lidar_omg, int &orig_odom_freq,
                                         int &cut_frame_num, QD &lidar_q,
                                         QD &imu_q,
  ...
  if (frame_num % orig_odom_freq * cut_frame_num == 0) {
    ...
    std::vector<double> EigenMat_1_col{EigenMatCwise(0, 0), EigenMatCwise(1, 0),
                                       EigenMatCwise(2, 0)};
    std::vector<double> EigenMat_2_col{EigenMatCwise(0, 1), EigenMatCwise(1, 1),
                                       EigenMatCwise(2, 1)};
    std::vector<double> EigenMat_3_col{EigenMatCwise(0, 2), EigenMatCwise(1, 2),
                                       EigenMatCwise(2, 2)};

    // Find the maximum value of each column
    int maxPos[3] = {0};
    maxPos[0] = max_element(EigenMat_1_col.begin(), EigenMat_1_col.end()) -
                EigenMat_1_col.begin();
    maxPos[1] = max_element(EigenMat_2_col.begin(), EigenMat_2_col.end()) -
                EigenMat_2_col.begin();
    maxPos[2] = max_element(EigenMat_3_col.begin(), EigenMat_3_col.end()) -
                EigenMat_3_col.begin();

    V3D Scaled_Eigen =
        EigenValue / data_accum_length; // the larger data_accum_length is, the
                                        // more data is needed
    ...
    double percentage_x =
        Rot_percent[axis[0]] < x_accumulate ? Rot_percent[axis[0]] : 1;
    double percentage_y =
        Rot_percent[axis[1]] < y_accumulate ? Rot_percent[axis[1]] : 1;
    double percentage_z =
        Rot_percent[axis[2]] < z_accumulate ? Rot_percent[axis[2]] : 1;
    ...
    fflush(stdout);
    if (Rot_percent[axis[0]] > x_accumulate &&
        Rot_percent[axis[1]] > y_accumulate &&
        Rot_percent[axis[2]] > z_accumulate) {
      printf(BOLDCYAN "[calibration] Data accumulation finished, Lidar IMU "
                      "calibration begins.\n\n" RESET);
      printf(BOLDBLUE "========================================================"
                      "==== \n\n" RESET);
...
void Gril_Calib::printProgress(double percentage, int axis_ascii) {
  ...
  if (percentage < 1) {
    ...
    printf(BOLDGREEN "Rotation around Lidar %c Axis complete! ",
           char(axis_ascii));
    cout << RESET << "\n";
  }
}
...
void Gril_Calib::LI_Calibration(int &orig_odom_freq, int &cut_frame_num,
  ...
  print_calibration_result(time_L_I, Rot_Lidar_wrt_IMU, Trans_Lidar_wrt_IMU,
...
void Gril_Calib::print_calibration_result(double &time_L_I, M3D &R_L_I,
                                          V3D &p_L_I, V3D &bias_g, V3D &bias_a,
                                          V3D gravity) {
  cout.setf(ios::fixed);
  printf(BOLDCYAN "[Calibration Result] " RESET);
  cout << setprecision(6)
       << "Rotation matrix from LiDAR frame to IMU frame    = "
       << RotMtoEuler(R_L_I).transpose() * 57.3 << " deg" << endl;
  printf(BOLDCYAN "[Calibration Result] " RESET);
  cout << "Translation vector from LiDAR frame to IMU frame = "
       << p_L_I.transpose() << " m" << endl;
  printf(BOLDCYAN "[Calibration Result] " RESET);
  printf("Time Lag IMU to LiDAR    = %.8lf s \n", time_L_I);
  printf(BOLDCYAN "[Calibration Result] " RESET);
  cout << "Bias of Gyroscope        = " << bias_g.transpose() << " rad/s"
       << endl;
  printf(BOLDCYAN "[Calibration Result] " RESET);
  cout << "Bias of Accelerometer    = " << bias_a.transpose() << " m/s^2"
       << endl;
}

void Gril_Calib::plot_result() {
  vector<vector<double>> IMU_omg(3), IMU_acc(3), IMU_ang_acc(3), Lidar_omg(3),
      Lidar_acc(3), Lidar_ang_acc(3), Lidar_vel(3), Lidar_pos(3);
  for (auto it_IMU_state = IMU_state_group.begin();
       it_IMU_state != IMU_state_group.end() - 1; it_IMU_state++) {
    for (int i = 0; i < 3; i++) {
