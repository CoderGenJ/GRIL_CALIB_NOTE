...
struct Angular_Vel_Cost_only_Rot {
  ...
  template <typename T> bool operator()(const T *q, T *residual) const {
    ...
    Eigen::Matrix<T, 3, 3> R_LI =
        q_LI.toRotationMatrix(); // Rotation (from LiDAR to IMU)
  ...
  static ceres::CostFunction *Create(const V3D IMU_ang_vel_,
                                     const V3D Lidar_ang_vel_) {
...
struct Angular_Vel_IL_Cost {
  ...
  Angular_Vel_IL_Cost(V3D IMU_ang_vel_, V3D IMU_ang_acc_, V3D Lidar_ang_vel_,
                      double deltaT_LI_)
  ...
  static ceres::CostFunction *Create(const V3D IMU_ang_vel_,
    ...
    return (new ceres::AutoDiffCostFunction<Angular_Vel_IL_Cost, 3, 4, 3, 1>(
        new Angular_Vel_IL_Cost(IMU_ang_vel_, IMU_ang_acc_, Lidar_ang_vel_,
                                deltaT_LI_)));
  }
...
struct Ground_Plane_Cost_IL {
  ...
  Ground_Plane_Cost_IL(QD Lidar_wrt_ground_, QD IMU_wrt_ground_,
      ...
      : Lidar_wrt_ground(Lidar_wrt_ground_), IMU_wrt_ground(IMU_wrt_ground_),
        distance_Lidar_wrt_ground(distance_Lidar_wrt_ground_),
        imu_sensor_height(imu_sensor_height_) {}
  ...
  bool operator()(const T *q, const T *trans, T *residual) const {
    ...
    Eigen::Matrix<T, 3, 3> R_GL =
        Lidar_wrt_ground_T
            .toRotationMatrix(); // Rotation matrix (from Ground to LiDAR)
    Eigen::Quaternion<T> IMU_wrt_ground_T =
        IMU_wrt_ground
            .cast<T>(); // from ground frame (earth frame) to IMU frame
    Eigen::Matrix<T, 3, 3> R_GI =
        IMU_wrt_ground_T
            .toRotationMatrix(); // Rotation matrix (from Ground to IMU)

    T distance_Lidar_wrt_ground_T =
        T(distance_Lidar_wrt_ground); // Distance of LiDAR plane from ground
    T imu_sensor_height_T =
        T(imu_sensor_height); // Height of IMU sensor from ground

    // Unknown Parameters, needed to be estimated
    Eigen::Quaternion<T> q_IL{q[0], q[1], q[2], q[3]};
    Eigen::Matrix<T, 3, 3> R_IL =
        q_IL.toRotationMatrix(); // Rotation IMU frame to LiDAR frame
    Eigen::Matrix<T, 3, 1> T_IL{trans[0], trans[1],
  ...
  static ceres::CostFunction *Create(const QD Lidar_wrt_ground_,
                                     ...
                                     const double distance_Lidar_wrt_ground,
...
struct Linear_acc_Rot_Cost_without_Gravity {
  ...
  Linear_acc_Rot_Cost_without_Gravity(CalibState LidarState_,
                                      V3D IMU_linear_acc_, QD Lidar_wrt_ground_)
      : LidarState(LidarState_), IMU_linear_acc(IMU_linear_acc_),
        Lidar_wrt_ground(Lidar_wrt_ground_) {}

  template <typename T>
  bool operator()(const T *q, const T *b_a, const T *trans, T *residual) const {
    // Known parameters used for Residual Construction
    Eigen::Matrix<T, 3, 3> R_LL0_T = LidarState.rot_end.cast<T>();
    Eigen::Matrix<T, 3, 1> IMU_linear_acc_T =
        IMU_linear_acc.cast<T>(); // Linear acceleration of IMU
    Eigen::Matrix<T, 3, 1> Lidar_linear_acc_T =
        LidarState.linear_acc.cast<T>(); // lidar linear acceleration
    Eigen::Quaternion<T> Lidar_wrt_ground_T =
        Lidar_wrt_ground.cast<T>(); // from ground frame to LiDAR plane frame
    Eigen::Matrix<T, 3, 3> R_GL =
        Lidar_wrt_ground_T.toRotationMatrix(); // Rotation matrix from Ground
                                               // frame to LiDAR frame

    // Unknown Parameters, needed to be estimated
    Eigen::Matrix<T, 3, 1> bias_aL{b_a[0], b_a[1],
                                   b_a[2]}; // Bias of Linear acceleration
    Eigen::Matrix<T, 3, 1> T_IL{trans[0], trans[1],
                                trans[2]}; // Translation of I-L (IMU wtr Lidar)
    Eigen::Quaternion<T> q_IL{q[0], q[1], q[2],
                              q[3]}; // Rotation from IMU frame to LiDAR frame
  ...
  static ceres::CostFunction *Create(const CalibState LidarState_,
                                     const V3D IMU_linear_acc_,
                                     const QD Lidar_wrt_ground_) {
    // residual 3, quaternion 4, bias 3, translation 3
    return (new ceres::AutoDiffCostFunction<Linear_acc_Rot_Cost_without_Gravity,
                                            3, 4, 3, 3>(
        new Linear_acc_Rot_Cost_without_Gravity(LidarState_, IMU_linear_acc_,
                                                Lidar_wrt_ground_)));
  }
...
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ofstream fout_LiDAR_meas, fout_IMU_meas, fout_before_filt_IMU,
      fout_before_filt_Lidar, fout_acc_cost, fout_after_rot;
  ...
  void push_ALL_IMU_CalibState(const sensor_msgs::Imu::ConstPtr &msg,
  ...
  void push_Plane_Constraint(const Eigen::Quaterniond &q_lidar,
  ...
  void Butter_filt(const deque<CalibState> &signal_in,
                   deque<CalibState> &signal_out);

  void zero_phase_filt(const deque<CalibState> &signal_in,
                       deque<CalibState> &signal_out);
  ...
  void set_states_2nd_filter(const deque<CalibState> &IMU_states,
  ...
  void align_Group(const deque<CalibState> &IMU_states,
  ...
  bool data_sufficiency_assess(MatrixXd &Jacobian_rot, int &frame_num,
  ...
  inline double get_time_result() { return time_offset_result; }
...
private:
  deque<CalibState> IMU_state_group; // LiDAR와 interpolation을 진행한 IMU data
                                     // 결과를 가지고 있는 groups
  deque<CalibState> Lidar_state_group; // LiDAR state
  deque<CalibState>
      IMU_state_group_ALL; // 모든 IMU data (ROS topic)를 가지고 있는 groups
  deque<Eigen::Quaterniond>
      Lidar_wrt_ground_group; // from ground frame (earth frame) to LiDAR plane
                              // frame
  deque<Eigen::Quaterniond>
      IMU_wrt_ground_group; // from ground frame (earth frame) to IMU frame
  deque<V3D> normal_vector_wrt_lidar_group; // normal vector of LiDAR ground
                                            // segmentation
  deque<double> distance_Lidar_wrt_ground_group; // distance from ground frame
                                                 // (earth frame) to LiDAR frame
  V3D Grav_L0; // Gravity vector in the initial Lidar frame L_0

  /// Parameters needed to be calibrated
  M3D Rot_Grav_wrt_Init_Lidar; // Rotation from inertial frame G to initial
                               // Lidar frame L_0
  ...
  double time_delay_IMU_wtr_Lidar; //(Soft) time delay between IMU and Lidar =
                                   // time_lag_1 + time_lag_2
  ...
  int lag_IMU_wtr_Lidar; // positive: timestamp of IMU is larger than that of
                         // LiDAR
