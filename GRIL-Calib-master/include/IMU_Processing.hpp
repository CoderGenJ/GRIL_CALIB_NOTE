...
private:
  ...
  void propagation_and_undist(const MeasureGroup &meas,
                              StatesGroup &state_inout,
                              PointCloudXYZI &pcl_in_out);
  void Forward_propagation_without_imu(const MeasureGroup &meas,
                                       StatesGroup &state_inout,
...
void ImuProcess::set_R_LI_cov(const V3D &R_LI_cov) { cov_R_LI = R_LI_cov; }

void ImuProcess::set_T_LI_cov(const V3D &T_LI_cov) { cov_T_LI = T_LI_cov; }
...
void ImuProcess::propagation_and_undist(const MeasureGroup &meas,
                                        StatesGroup &state_inout,
                                        PointCloudXYZI &pcl_out) {
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
    ...
    V3D angvel_now(head->angular_velocity.x, head->angular_velocity.y,
                   head->angular_velocity.z);
    V3D acc_now(head->linear_acceleration.x, head->linear_acceleration.y,
                head->linear_acceleration.z);
    fout_imu << setw(10) << head->header.stamp.toSec() << "  "
             << angvel_now.transpose() << " " << acc_now.transpose() << endl;
    ...
    cov_w.block<3, 3>(12, 12) =
        R_imu * cov_acc.asDiagonal() * R_imu.transpose() * dt * dt;
    cov_w.block<3, 3>(15, 15).diagonal() =
  ...
  if (lidar_type != L515) {
    auto pos_liD_e =
        state_inout.pos_end + state_inout.rot_end * state_inout.offset_T_L_I;
    // auto R_liD_e   = state_inout.rot_end * Lidar_R_to_IMU;

#ifdef DEBUG_PRINT
    cout << "[ IMU Process ]: vel " << state_inout.vel_end.transpose()
         << " pos " << state_inout.pos_end.transpose() << " ba"
         << state_inout.bias_a.transpose() << " bg "
         << state_inout.bias_g.transpose() << endl;
    cout << "propagated cov: " << state_inout.cov.diagonal().transpose()
         << endl;
#endif
    ...
    for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--) {
      ...
      for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
        ...
        V3D T_ei(pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt +
                 R_i * state_inout.offset_T_L_I - pos_liD_e);
    ...
    if (imu_need_init_) {
      if (!Gril_Calib_done) {
        ...
        if (init_iter_num > MAX_INI_COUNT) {
          ...
          ROS_INFO("IMU Initialization Done: Gravity: %.4f %.4f %.4f, Acc "
                   "norm: %.4f",
        ...
        cout << endl;
        printf(BOLDMAGENTA "[Refinement] Switch to LIO mode, online refinement "
                           "begins.\n\n" RESET);
        last_imu_ = meas.imu.back();
        imu_need_init_ = false;
        cov_acc = cov_acc_scale;
