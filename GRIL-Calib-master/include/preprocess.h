...
public:
  ...
  void process_cut_frame_livox(const livox_ros_driver::CustomMsg::ConstPtr &msg,
  ...
  void process(const sensor_msgs::PointCloud2::ConstPtr &msg,
               PointCloudXYZI::Ptr &pcl_out);
  void process_cut_frame_pcl2(const sensor_msgs::PointCloud2::ConstPtr &msg,
                              deque<PointCloudXYZI::Ptr> &pcl_out,
                              deque<double> &time_lidar,
                              const int required_frame_num, int scan_count);
...
private:
  ...
  // bool small_plane(const PointCloudXYZI &pl, vector<orgtype> &types, uint
  // i_cur, uint &i_nex, Eigen::Vector3d &curr_direct);
